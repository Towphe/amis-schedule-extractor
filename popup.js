...
function extractSchedule(amisDOM) {
  ...
  setTimeout(() => {
    enlistedSubjectsDOM = doc.querySelector(
      "#active-enlistment > div.bg-white.shadow-lg.sm\\:rounded-lg.my-3.overflow-auto > div",
    ).children;

    let i = 0;
    for (const esd of enlistedSubjectsDOM) {
      if (i === 0) {
        // skip table header
        i++;
        continue;
      ...
      // esd[1] contains enlistment status
      ...
      if (
        subjectInfoDOM[1].children[0].children[0].innerHTML.trim() !=
          "Enlisted" &&
        subjectInfoDOM[1].children[0].children[0].innerHTML.trim() !=
          "Finalized"
      ) {
      ...
      let lectureTitle = lectureInfoDOM
      ...
      enlistedSubject.name = splittedLecture[0].trim();
      enlistedSubject.lectureSection = splittedLecture[1].trim();

      let units = lectureInfoDOM
        .getElementsByTagName("button")[0]
        .getElementsByTagName("div")[0]
        .getElementsByTagName("div")[1]
        .getElementsByTagName("div")[0].innerHTML;

      enlistedSubject.units = parseInt(units.trim().split(" ")[0].trim());

      // extract DOM of lecture content [0] contains time and days, [1] contains faculty and location
      const lectureBody =
        subjectInfoDOM[0].children[0].children[1].children[0].children[0];

      let lectureTime = lectureBody.children[0].innerText
        .trim()
        .split(": ")[1]
        .substring(1, 18);
      ...
      let lectureDays =
        lectureBody.children[0].children[1].getElementsByTagName("div");
      enlistedSubject.lectureDays = [];
      for (let day of lectureDays) {
        enlistedSubject.lectureDays.push(day.innerText.trim());
      }

      enlistedSubject.lectureFIC = lectureBody.children[1].children[0].innerText
        .split(": ")[1]
        .trim();
      enlistedSubject.lectureLocation =
      ...
      if (labInfoDOM.innerHTML.trim() !== "-- No associated class --") {
        // process lab info
        enlistedSubject.laboratorySection =
          labInfoDOM.children[0].children[0].children[0].innerHTML
            .trim()
            .split("-")[1]
            .trim();

        // extract DOM of lab content [0] contains time and days, [1] contains faculty and location
        // const labBody =
        //   subjectInfoDOM[1].children[0].children[1].children[0].children[0];

        const labBody = labInfoDOM.children[1].children[0].children[0];

        let labTime = labBody.children[0].innerText
          .trim()
          .split(": ")[1]
          .substring(1, 18);
        ...
        enlistedSubject.labFIC = labBody.children[1].children[0].innerText
          .split(": ")[1]
          .trim();
        enlistedSubject.labLocation = labBody.children[1].children[1].innerText
          .split(": ")[1]
          .trim();
      } else {
...
function getSemesterDates() {
  ...
  if (month >= 7 && month <= 11) {
    ...
    // Jan - Jun
    startDate = new Date(year, 0, 27); // January 27
...
async function initialize() {
  if (
    ...
    location.href.endsWith("popup.html")
  ) {
    let amisDOM;
      ...
      chrome.scripting.executeScript(
        ...
        (results) => {
          ...
          if (amisDOM) {
            ...
            const buttons = [
              document.getElementById("csv-dl"),
              document.getElementById("json-dl"),
              document.getElementById("gcal-dl"),
            ];
    ...
    document.getElementById("json-dl").addEventListener("click", () => {
      ...
      JSONToFile(jsonSchedule, "2S2425-SCHED");
    });
  }
}
